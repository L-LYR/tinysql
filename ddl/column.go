...
func onDropColumn(t *meta.Meta, job *model.Job) (ver int64, _ error) {
	...
	case model.StatePublic:
		...
		// I think the test cases cannot detect the wrong location of adjustColumnInfoInDropColumn.
		adjustColumnInfoInDropColumn(tblInfo, colInfo.Offset)
		// corner case about the default value
		// During the precedure, if it need rollback, we will depend on the default value of column.
		if colInfo.GetOriginDefaultValue() == nil && mysql.HasNotNullFlag(colInfo.Flag) {
			defaultVal, err := generateOriginDefaultValue(colInfo)
			if err != nil {
				return ver, errors.Trace(err)
			}
			colInfo.SetDefaultValue(defaultVal)
		}
